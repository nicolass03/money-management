...
- **Invite/set-password:** `/set-password` must call `establishAuthSessionFromUrl()` to exchange PKCE `?code=` **before** clearing URL params. Clearing `code` early leaves a stale `localStorage` JWT and `updateUser` fails with Supabase `user_not_found` (“User from sub claim in JWT does not exist”). Re-test invites in a fresh browser profile after deleting test users in the dashboard.
